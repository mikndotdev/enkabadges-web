    ...
    return (
        <div className="card w-1/2 bg-blue-500 shadow-xl py-5 space-y-4">
            ...
            <div className="flex flex-row justify-center space-x-3">
                <button className="btn btn-accent disabled:btn-error" disabled={game === "genshin"}
                        onClick={() => setGame("genshin")}>
                    Genshin Impact
                    <img src="/genshin.webp" alt="Genshin Impact" className="w-10 h-10 rounded-full"/>
                </button>
                <button className="btn btn-accent disabled:btn-error" disabled={game === "hsr"}
                        onClick={() => setGame("hsr")}>
                    Honkai: Star Rail
            ...
            <div className="flex flex-row justify-center space-x-3">
                <input type="text" placeholder="UID" value={uid} maxLength={9}
                       className="input input-bordered w-full max-w-96 justify-center "
                       onChange={e => setUid(e.target.value)}/>
            </div>
            <h1 className="text-2xl font-bold text-center">Pick a color for your badge</h1>
            <div className="flex flex-row justify-center space-x-3">
                <ColorPicker color={color} onChange={setColor} hideInput={["rgb", "hsv"]} hideAlpha height={100}/>
            </div>
        </div>
    );
}